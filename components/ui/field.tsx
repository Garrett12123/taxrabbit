...
// Context for field error state
const FieldErrorContext = createContext<{ hasError: boolean }>({ hasError: false })
...
const fieldVariants = cva("data-[invalid=true]:text-destructive gap-3 group/field flex w-full transition-all duration-150", {
  variants: {
    orientation: {
      vertical:
...
type FieldProps = React.ComponentProps<"div"> & 
  VariantProps<typeof fieldVariants> & {
    /** When true, applies error styling and shake animation */
    error?: boolean
  }
...
function Field({
  ...
  return (
    <FieldErrorContext.Provider value={{ hasError: error }}>
      <div
        role="group"
        data-slot="field"
...
/** Hook to check if parent Field has error */
...
function FieldError({
  ...
  return (
    <div
      ...
      className={cn(
        "text-destructive text-sm font-normal animate-error-in",
        className
      )}
      {...props}
    >
...
/** Success or info message for a field */
function FieldMessage({
  ...
  return (
    <div
      ...
      className={cn(
        "text-sm font-normal animate-error-in",
